...
const char RBx_PIN = 8;
const char IF_PIN = 0;

#define max_buffer_size 4000

unsigned int delay_buffer[max_buffer_size];
...
void InitMain() {
    ...
    TRISB.AN10_PIN = 1;  // Set AN0 as input
    TRISB.RBx_PIN = 0;  // Set RBx as output
    LATB.RBx_PIN = 0;
    //ADC1_Init();
    ADC_Init_Manual();
...
void Timer1Int() iv IVT_ADDR_T1INTERRUPT {
    ...
    W4 = analog_data;
    W5 = delay_buffer[iterator];
    W6 = 32767 - pot_value; 
    W7 = pot_value;
    CORCON.IF_PIN = 0;
    asm{
        clr A
        mac W4*W6, A, W4, W4, W6, W6
        sac A, #0, W1
        clr B
        mac W5*W7, B, W5, W5, W7, W7
        sac B, #0, W2
    }
    output_data = W1 + W2;
    CORCON.IF_PIN = 1;
    delay_buffer[iterator++] = output_data;


    if (iterator >= buffer_size) iterator = 0;
